...
if [ "$cmd" == "check" ]; then
  ...
  if [ "$DATE_TIME_TARGET" == "READY" ]; then
    ...
    REMAINING=$(datediff now "${DATE_TIME_TARGET}" -f '%0H:%0M:%0S' --from-zone="$TIME_ZONE")
  ...
  TIMER_TARGET=$(zenity --scale --title "Set timer" --text "In x minutes:" --min-value=1 --max-value=600 --step=1 --value=60)
  if [ "$?" = 0 ]; then
